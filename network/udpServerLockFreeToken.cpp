...
moodycamel::ProducerToken produceToken(message_queue);
moodycamel::ConsumerToken consumerToken(message_queue);
...
void workerThread(int index) {
    while (true) {
        std::string message;
        if (message_queue.try_dequeue(consumerToken, message)) {  // Non-blocking dequeue
    ...
    while (true) {
        ...
        for (int i = 0; i < event_count; i++) {
            if (events[i].data.fd == sockfd) {
                ...
                // Lock-free enqueue
                message_queue.enqueue(produceToken, message);
            }
        }
    }
